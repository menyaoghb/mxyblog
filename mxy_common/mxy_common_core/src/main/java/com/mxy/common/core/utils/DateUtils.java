...
public class DateUtils {
     ...
     * 转换时间
     */
    public static String getDate(Date date, String timeLength) {
        String deadLine = "";
        if (date != null) {
            String dateString = format(date, "yyyy-MM-dd HH:mm:ss");
            double dateTime = Double.parseDouble(timeLength);
            int minute = (int) (dateTime * 60);
            deadLine = addDateTime(dateString, minute);
        }
        return deadLine;
    }
     ...
     * 时间累加--小时
     */
    public static String addDateTime(String day, int minute) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date date = null;
        try {
            // 输入的日期
